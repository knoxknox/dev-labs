...
int main(int argc, char *argv[])
{
  ...
  wait(NULL); // SIGTRAP wait (attaching is complete)
  ...
  inject(target, shellcode, (void*)regs.rip, SHELLCODE_SIZE);

  // We had modified the instruction pointer
  // PTRACE_DEATCH subtracts 2 bytes to the IP
  regs.rip += 2;
